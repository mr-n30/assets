...
sudo $BIN_PATH/subfinder -v -d $DOMAIN -o $OUTPUT_DIR/subfinder.txt -all -config $SUBFINDER_CONFIG
sleep 3
...
sudo $BIN_PATH/shuffledns -v -d $DOMAIN -w $DNS_BRUTE -o $OUTPUT_DIR/shuffledns.txt -r $RESOLVERS -wt 100
sleep 3
...
cat $OUTPUT_DIR/*.txt | sort -u | sudo $BIN_PATH/dnsx -a -resp -verbose -o $OUTPUT_DIR/dnsx.out
...
# Begin screenshoting with aquatone
echo -e "${MAGENTA}${BOLD}############################################${END}${END}"
echo -e "${MAGENTA}${BOLD}### ${YELLOW}[+] Running: aquatone${END}${END}${END}"
echo -e "${MAGENTA}${BOLD}############################################${END}${END}"

mkdir -p $OUTPUT_DIR/aquatone-basic
sudo aquatone -chrome-path /usr/bin/google-chrome -out $OUTPUT_DIR/aquatone-basic < $OUTPUT_DIR/httpx.txt
sleep 3
...
sudo masscan -v --rate=1000 -p$(cat $NMAP_TOP_PORTS) -oG $OUTPUT_DIR/masscan-output.gnmap -iL $OUTPUT_DIR/ips.txt
sleep 3
...
sudo nmap -v -sC -sV -p$PORTS -oA $OUTPUT_DIR/nmap/nmap -iL $OUTPUT_DIR/all.txt
sleep 3
...
sudo aquatone -chrome-path /usr/bin/google-chrome -out $OUTPUT_DIR/aquatone -nmap < $OUTPUT_DIR/nmap/nmap.xml
sleep 3
...
sudo brutespray -c --file $OUTPUT_DIR/nmap/nmap.gnmap -o $OUTPUT_DIR/brutespray/ --threads 9 --hosts 3
sleep 3
...
sudo dirsearch -l $OUTPUT_DIR/httpx.txt -w $WORDLIST -i 200,405,500 -e .php,/,.txt --plain-text-report $OUTPUT_DIR/dirsearch/dirsearch.txt \
	--full-url \
	--force-extensions \
	-H 'X-Remote-IP: 127.0.0.1' \
	-H 'X-Remote-Addr: 127.0.0.1' \
